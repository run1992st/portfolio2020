<template>
  <div id="app" class="relative h-screen w-screen overflow-y-scroll">
    <ThePopup
      class="fixed top-0 left-0 z-50"
      :popup="popup"
      @remove-popup="popup=false"
      :images="images"
    />
    <TheNavBar class="fixed w-full top-0 z-20" />
    <TheLanding />
    <TheCardsContainer id="card-stack-section" class="pt-32 flex flex-col" @popup="popup=!popup" />
    <TheFooter class="mt-12 mb-24 h-64"></TheFooter>
  </div>
</template>

<script>
import TheNavBar from "@/components/TheNavBar.vue";
import TheLanding from "@/components/TheLanding.vue";
import ThePopup from "@/components/ThePopup.vue";
import TheCardsContainer from "@/components/TheCardsContainer.vue";
import TheFooter from "@/components/TheFooter.vue";

export default {
  name: "App",
  components: { TheNavBar, TheLanding, ThePopup, TheCardsContainer, TheFooter },
  data() {
    return {
      images: [
        { categoryName: "Adobe Illustrator", path: "Photos/Drawing-01.jpg" },
        { categoryName: "Adobe Illustrator", path: "Photos/Jazz_is_Free.jpg" },
        { categoryName: "Adobe Illustrator", path: "Photos/Packaging_label.jpg" },
        { categoryName: "Adobe Photoshop", path: "Photos/Fisherfolkinbkk.jpg" },
        {
          categoryName: "Coporate Identity_Guideline",
          path: "Photos/Coporate_Guideline.jpg",
        },
        {
          categoryName: "Interior Photographing/Lightroom",
          path: "Photos/Interior_1.jpg",
        },
        {
          categoryName: "Interior Photographing/Lightroom",
          path: "Photos/Interior_2.jpg",
        },
        {
          categoryName: "Interior Photographing/Lightroom",
          path: "Photos/Interior_3.jpg",
        },
        {
          categoryName: "Interior Photographing/Lightroom",
          path: "Photos/Interior_4.jpg",
        },
      ],
      popup: false,
    };
  },
};
</script>

<style>
#app {
  scroll-behavior: smooth;
}
#app::before {
  content: " ";
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  background-image: url(data:image/png;base64,);
  background-blend-mode: darken;
  opacity: 8%;
}

::-webkit-scrollbar {
  z-index: 99;
  width: 0.925rem;
  border-radius: 8px;
}
::-webkit-scrollbar-track {
  z-index: 99;
  -webkit-box-shadow: inset 0 0 4px rgba(0, 0, 0, 0.03);
  box-shadow: inset 0 0 4px rgba(0, 0, 0, 0.03);
  background-color: rgb(250, 250, 250);
  border-radius: 8px;
}
::-webkit-scrollbar-thumb:window-inactive {
  background-color: rgb(236, 235, 235);
  border: 1px solid #f5f4f4;
  border-radius: 8px;
}
::-webkit-scrollbar-thumb {
  z-index: 99;
  background-color: rgb(233, 233, 233);
  border: 1px solid #f5f4f4;
  border-radius: 8px;
}
</style>
